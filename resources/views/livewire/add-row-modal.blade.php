                        ...
                        <flux:select wire:model="formData.{{ $field }}" placeholder="Pilih {{ ucfirst(str_replace('_', ' ', $field)) }}">
                            ...
                            @foreach($options as $option)
                                ...
                                @elseif($field === 'idkode_tindakan_terapi')
                                    @php
                                        $label = $option->kode ?? $option->getKey();
                                        $name = $option->nama_tindakan ?? $option->deskripsi_tindakan_terapi ?? $this->getDisplayName($option);
                                    @endphp
                                    <option value="{{ $option->getKey() }}">{{ $label }} - {{ $name }}</option>
                                @elseif($field === 'id_user')
