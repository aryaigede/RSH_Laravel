...
<div class="space-y-8">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="lg:col-span-1 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-6 shadow-sm">
            <h3 class="text-base font-semibold text-zinc-900 dark:text-zinc-100 mb-4">Profil Dokter</h3>
            <div class="space-y-3 text-sm text-zinc-700 dark:text-zinc-300">
                ...
                <div class="flex justify-between"><span>Bidang</span><span class="font-semibold">{{ $doctorProfile->bidang_dokter ?? '-' }}</span></div>
                <div class="flex justify-between"><span>No. HP</span><span class="font-semibold">{{ $doctorProfile->no_hp ?? '-' }}</span></div>
                <div class="flex justify-between"><span>Alamat</span><span class="font-semibold text-right">{{ $doctorProfile->alamat ?? '-' }}</span></div>
            </div>
        </div>
        ...
        <div class="lg:col-span-2 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-6 shadow-sm">
            <div class="flex justify-between items-center mb-4">
                <h3 class="text-base font-semibold text-zinc-900 dark:text-zinc-100">Pasien Anda</h3>
            </div>
                ...
                <tbody class="divide-y divide-zinc-200 dark:divide-zinc-800 text-zinc-800 dark:text-zinc-200">
                    @forelse($rekamMedis as $rm)
                        <tr>
                            ...
                            <td class="py-2 pr-4">{{ optional($rm->created_at)->format('Y-m-d') }}</td>
                        </tr>
                    @empty
